...
<meta name="viewport"
	content="width=device-width,initial-scale=1,minimum-scale=1.0,maximum-scale=1.0,loupanimg-scalable=no" />
...
<body>
	<nav class="breadcrumb">
		...
		楼盘图片管理 <span class="c-gray en">&gt;</span> 楼盘图片列表 <a
	...
	<div class="page-container">
		<div class="text-c">
			...
			<input type="text" class="input-text" style="width: 250px;height:2.4em"
				placeholder="输入楼盘图片名称" id="" name="">
			<button type="submit" class="btn btn-success" style="height:2.4em" id="" name="">
				<i class="Hui-iconfont">&#xe665;</i> 搜楼盘图片
			</button>
		</div>
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l">
				<a href="javascript:;" onclick="datadel();" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> 
				<a href="javascript:;" onclick="loupanimg_add('添加楼盘图片','<%=path%>/lpimg/initaddloupanimg','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加楼盘图片</a>
		...
		<table
			...
			<thead>
				<tr>
					<th scope="col" colspan="9">楼盘图片列表</th>
				</tr>
				<tr class="text-c">
					<th width="25"><input type="checkbox" name="id" value=""></th>
					<th width="100">楼盘图片</th>
			...
			<tbody id="pageBody">
				<s:iterator var="l" value="#request.loupanimglist">
					<tr class="text-c">
						<td>
							<input type="checkbox" value="${l.idString}" name="id">
						</td>
						<td><a href="javascript:;" onClick="picture_edit('图库编辑','<%=path%>/lpimg/showloupanimg','${l.loupan.nameString}')"><img style="width:40px;height:40px;border-radius:100%" class="picture-thumb" src="<%=path%>/${l.imgPathString}"></a></td>
						<td>
							<a style="cursor: pointer;text-decoration: none;" class="text-primary" onclick="loupan_show('${l.loupan.nameString}楼盘详情','<%=path%>/lp/showloupan?loupan.idString=${l.buildingsIdString}','${l.loupan.nameString}','600','430')">${l.loupan.nameString}</a>
						</td>
						<td>${l.createdTime}</td>
						<td class="td-manage">
							<a title="删除楼盘图片" href="javascript:;" onclick="loupanimg_del(this,'${l.idString}')"	class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6e2;</i></a>
						</td>
					</tr>
				</s:iterator>
			</tbody>
			...
			<div class="row-fluid">
				<div class="span6">
					<div class="dataTables_info" id="DataTables_Table_0_info">
						...
						<select name="loupanimg.houseTypeString" class="select" style="width:80px;margin-top:0" onchange="checkpage(this.value);">
				...
				<div class="span6">
					<div class="dataTables_paginate paging_bootstrap pagination">
						<ul>
							<li class="prev">
								<a href="<%=path%>/lpimg/pageloupanimg?currentpage=1&handle=firstpage">← 第一页</a>
							</li>
							<li class="prev">
								<a href="<%=path%>/lpimg/pageloupanimg?currentpage=${pager.currentpage}&handle=uppage">← 上一页</a>
							</li>
<!-- 							<li> -->
<%-- 								<a href="<%=path%>/lp/pageloupanimg?currentpage=${n}">${n}</a> --%>
<!-- 							</li> -->
							<li class="next">
								<a href="<%=path%>/lpimg/pageloupanimg?currentpage=${pager.currentpage}&handle=downpage">下一页 → </a>
							</li>
							<li class="next">
								<a href="<%=path%>/lpimg/pageloupanimg?currentpage=${pager.sumpage}&handle=lastpage">尾页 → </a>
							</li>
						</ul>
					</div>
				</div>
...
<script type="text/javascript">
		...
		function checkpage(index){
			window.location.href='${pageContext.request.contextPath}/lpimg/pageloupanimg?currentpage='+index;
		}
		/*图片-编辑*/
		...
		/*楼盘图片-刷新*/
		function refresh(){
			window.location.href='${pageContext.request.contextPath}/lpimg/pageloupanimg';
			layer.msg('刷新成功!', {
				icon : 1,
				time : 2000
			});
		...
		function datadel(){
			...
			window.location.href='${pageContext.request.contextPath}/lpimg/deletemanyloupanimg?id='+a;
			layer.msg('删除成功!', {
				icon : 1,
				time : 2000
			});
		}
		function loupanimg_add(title, url, w, h) {
			layer_show(title,url,w,h);
		}
		/*楼盘图片-删除*/
		function loupanimg_del(obj, id) {
			layer.confirm('确认要删除吗？',function(index) {
			$.ajax({
					type : 'POST',
					url : '${pageContext.request.contextPath}/lpimg/deleteimg?img.idString='+id,
					dataType : 'text',
		...
		/*楼盘图片-编辑*/
		function loupanimg_edit(title, url, id, w, h) {
			layer_show(title,url,w,h);
		}
		function loupanimg_addimg(title,url,id,w,h){
			layer_show(title,url,w,h);
		}
		/*楼盘-编辑*/
