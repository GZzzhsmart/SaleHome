		...
		<table
			...
			<thead>
				...
				<tr class="text-c">
					...
					<th width="60">房屋编号</th>
					<th width="60">折扣（折）</th>
					<th width="60">总价（万元）</th>
					<th width="60">销售员工</th>
					<th width="100">账单审核</th>
					<th width="60">销售状态</th>
					<th width="100">操作</th>
				</tr>
			</thead>
					...
					<tr class="text-c">
						...
						<td>
							...
							<c:if test="${l.saleHomeStatus==1}">
								审核通过<a style="cursor: pointer;text-decoration: none;" class="text-primary" onclick="sale_edit('${l.employee.nameString}员工账单详情','<%=path%>/sale/showsale?sale.idString=${l.idString}','${l.employee.nameString}','600','330')">（查账）</a>
							</c:if>
							<c:if test="${l.saleHomeStatus==2}">
								审核不通过<a style="cursor: pointer;text-decoration: none;" class="text-primary" onclick="sale_edit('${l.employee.nameString}员工账单详情','<%=path%>/sale/showsale?sale.idString=${l.idString}','${l.employee.nameString}','600','330')">（详情）</a>
							</c:if>
							<c:if test="${l.saleHomeStatus==3}">
								审核驳回<a style="cursor: pointer;text-decoration: none;" class="text-primary" onclick="sale_edit('${l.employee.nameString}员工账单详情','<%=path%>/sale/showsale?sale.idString=${l.idString}','${l.employee.nameString}','600','330')">（详情）</a>
							</c:if>
						</td>
						<c:if test="${l.statusInt==0}">
							...
							<td class="td-manage">
								...
								<c:if test="${l.saleHomeStatus!=1}">
									<a title="审核账单" href="javascript:;"	onclick="saleshenhe_list('${l.employee.nameString}员工请求审核账单详情','<%=path%>/sale/shenhesale?sale.idString=${l.idString}','${l.employee.nameString}','','500')" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i>审核账单</a> 							
								</c:if>
								<c:if test="${l.saleHomeStatus==1}">
									<a title="审核账单" href="javascript:;"	onclick="alert('账单已审核，资金已入库');" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i>资金入库</a> 							
								</c:if>
							</td>
						</c:if>
			...
			<div class="row-fluid">
				...
				<div class="span6">
					<div class="dataTables_paginate paging_bootstrap pagination">
						<ul>
							<li class="prev">
								<a href="<%=path%>/sale/shenhelist?currentpage=1&handle=firstpage&tbuilding.idString=${tbuilding.idString}">← 第一页</a>
							</li>
							<li class="prev">
								<a href="<%=path%>/sale/shenhelist?currentpage=${pager.currentpage}&handle=uppage&tbuilding.idString=${tbuilding.idString}">← 上一页</a>
							</li>
<!-- 							<li> -->
<%-- 								<a href="<%=path%>/lp/shenhelist?currentpage=${n}">${n}</a> --%>
<!-- 							</li> -->
							<li class="next">
								<a href="<%=path%>/sale/shenhelist?currentpage=${pager.currentpage}&handle=downpage&tbuilding.idString=${tbuilding.idString}">下一页 → </a>
							</li>
							<li class="next">
								<a href="<%=path%>/sale/shenhelist?currentpage=${pager.sumpage}&handle=lastpage&tbuilding.idString=${tbuilding.idString}">尾页 → </a>
							</li>
						</ul>
					</div>
				</div>
...
<script type="text/javascript">
		...
		function checkpage(index){
			window.location.href='${pageContext.request.contextPath}/sale/shenhelist?currentpage='+index;
		}
		/*销售记录-刷新*/
		function refresh(){
			window.location.href='${pageContext.request.contextPath}/sale/shenhelist?tbuilding.idString=${tbuilding.idString}';
			layer.msg('刷新成功!', {
				icon : 1,
				time : 2000
			});
...
// 			layer.full(index);
		...
		/*楼盘-编辑*/
		function saleshenhe_list(title, url, id, w, h) {
				layer_show(title,url,w,h);
		}
		/*销售记录-编辑*/
