...
<body>
	<nav class="breadcrumb">
		...
		文章类别管理 <span class="c-gray en">&gt;</span> 文章类别列表 <a
	...
	<div class="page-container">
		<div class="text-c">
			...
			<input type="text" class="input-text" style="width: 250px;height:2.4em"
				placeholder="输入文章类别名称" id="" name="">
			<button type="submit" class="btn btn-success" style="height:2.4em" id="" name="">
				<i class="Hui-iconfont">&#xe665;</i> 搜文章类别
			</button>
		</div>
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l">
				<a href="javascript:;" onclick="datadel();" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> 
				<a href="javascript:;" onclick="article_add('添加文章类别','<%=path%>/hui-jxs/articletype-add.jsp','','300')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加文章类别</a>
		...
		<table
			...
			<thead>
				<tr>
					<th scope="col" colspan="4">文章类别列表</th>
				</tr>
				<tr class="text-c">
					<th width="25"><input type="checkbox" name="" value=""></th>
					<th width="100">文章类别名称</th>
					<th width="100">文章类别描述</th>
					<th width="100"><i class="icon Hui-iconfont">&#xe68b;</i>操作</th>
				</tr>
			</thead>
			<tbody id="pageBody">
				<s:iterator var="l" value="#request.tArticleTypelist">
					<tr class="text-c">
						<td>
						...
						<td>${l.desString}</td>
						<td class="td-manage">
							<a title="编辑文章" href="javascript:;"	onclick="article_edit('${l.nameString}文章的修改','<%=path%>/wztype/initupdatearticletype?tArticleType.idString=${l.idString}','${l.nameString}','800','260')" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i></a> 
							<a title="删除文章类别" href="javascript:;" onclick="article_del(this,'${l.idString}')"	class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6e2;</i></a>
						</td>
					</tr>
				</s:iterator>
			</tbody>
			...
			<div class="row-fluid">
				...
				<div class="span6">
					<div class="dataTables_paginate paging_bootstrap pagination">
						<ul>
							<li class="prev">
								<a href="<%=path%>/wztype/pagearticletype?currentpage=1&handle=firstpage">← 第一页</a>
							</li>
							<li class="prev">
								<a href="<%=path%>/wztype/pagearticletype?currentpage=${pager.currentpage}&handle=uppage">← 上一页</a>
							</li>
<!-- 							<li> -->
<%-- 								<a href="<%=path%>/lp/articletypelist?currentpage=${n}">${n}</a> --%>
<!-- 							</li> -->
							<li class="next">
								<a href="<%=path%>/wztype/pagearticletype?currentpage=${pager.currentpage}&handle=downpage">下一页 → </a>
							</li>
							<li class="next">
								<a href="<%=path%>/wztype/pagearticletype?currentpage=${pager.sumpage}&handle=lastpage">尾页 → </a>
							</li>
						</ul>
					</div>
				</div>
...
<script type="text/javascript">
		...
		function checkpage(index){
			window.location.href='${pageContext.request.contextPath}/wztype/pagearticletype?currentpage='+index;
		}
		/*文章类别-刷新*/
		function refresh(){
			window.location.href='${pageContext.request.contextPath}/wztype/pagearticletype';
			layer.msg('刷新成功!', {
				icon : 1,
				time : 2000
			});
		...
		function datadel(){
			...
			window.location.href='${pageContext.request.contextPath}/wztype/deletemanyarticletype?id='+a;
			layer.msg('删除成功!', {
				icon : 1,
				time : 2000
			});
		...
		function article_del(obj, id) {
			...
			$.ajax({
					type : 'POST',
					url : '${pageContext.request.contextPath}/wztype/deletearticletype?tArticleType.idString='+ id,
					dataType : 'text',
...
// 			layer.full(index);
		...
		function article_stop(obj, id) {
			layer.confirm('确认禁用吗？',function(index) {
				...
				$.post(
					"${pageContext.request.contextPath}/wztype/updatearticletypestatus",
					{
						"tArticleType.idString" : id,
						"tArticleType.statusInt" : 1
					},
					function(data) {
					}, 
					"json"
		...
		function article_start(obj, id) {
			...
			function(index) {
				...
				$.post("${pageContext.request.contextPath}/wztype/updatearticletypestatus",
				{
					"tArticleType.idString" : id,
					"tArticleType.statusInt" : 0
				}, function(data) {
				}, "json");
			});
		}
