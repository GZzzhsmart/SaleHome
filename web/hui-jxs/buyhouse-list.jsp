			...
			<tbody id="pageBody">
				<s:iterator var="l" value="#request.appointmentlist">
					<tr class="text-c">
						...
						<td>
							<a style="cursor: pointer;text-decoration: none;" class="text-primary" onclick="user_edit('${l.house.nameString}户型详情','<%=path%>/appointment/showhouse?house.idString=${l.house.idString}','${l.house.nameString}','600','430')">${l.house.nameString}</a>
						</td>
						...
						<c:if test="${l.status==0}">
							...
							<td class="td-manage">
								...
								<c:if test="${l.appStatusInt==2}">
									<a title="编辑" href="javascript:;"	onclick="user_edit('${l.user.nameString}购房资料填写','<%=path%>/hui-jxs/user_addcustomer.jsp?id=${l.idString}','${l.idString}','','360')" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i>确认购房</a> 
								</c:if>
							</td>
						</c:if>
			...
			<div class="row-fluid">
				...
				<div class="span6">
					<div class="dataTables_paginate paging_bootstrap pagination">
						<ul>
							<li class="prev">
								<a href="<%=path%>/appointment/buyhouse?currentpage=1&handle=firstpage">← 第一页</a>
							</li>
							<li class="prev">
								<a href="<%=path%>/appointment/buyhouse?currentpage=${pager.currentpage}&handle=uppage">← 上一页</a>
							</li>
<!-- 							<li> -->
<%-- 								<a href="<%=path%>/lp/pageuser?currentpage=${n}">${n}</a> --%>
<!-- 							</li> -->
							<li class="next">
								<a href="<%=path%>/appointment/buyhouse?currentpage=${pager.currentpage}&handle=downpage">下一页 → </a>
							</li>
							<li class="next">
								<a href="<%=path%>/appointment/buyhouse?currentpage=${pager.sumpage}&handle=lastpage">尾页 → </a>
							</li>
						</ul>
					</div>
				</div>
...
<script type="text/javascript">
		...
		function checkpage(index){
			window.location.href='${pageContext.request.contextPath}/appointment/buyhouse?currentpage='+index;
		}
		/*我的预约-刷新*/
		function refresh(){
			window.location.href='${pageContext.request.contextPath}/appointment/buyhouse';
			layer.msg('刷新成功!', {
				icon : 1,
				time : 2000
			});
...
// 			layer.full(index);
		...
		function buyhouse(obj, id) {
			layer.confirm('确认购房吗？',function(index) {
				layer.msg('购房预订成功，等待和销售顾问进行交接!', {
					icon : 6,
					time : 1000
				});
				$.post(
					"${pageContext.request.contextPath}/appointment/querenbuy",
					{
						"appointment.idString" : id
					},
					function(data) {
