	...
	<div class="page-container">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l">
				...
				<a href="javascript:;" onclick="customer_add('添加客户','<%=path%>/hui-jxs/customer-add.jsp','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加客户</a>
		...
		<table
			...
			<thead>
				<tr>
					<th scope="col" colspan="10">客户列表</th>
				</tr>
				<tr class="text-c">
			...
			<tbody id="pageBody">
				<s:iterator var="l" value="#request.customerlist">
					<tr class="text-c">
						...
						<td>
							<a style="cursor: pointer;text-decoration: none;" class="text-primary" onclick="mycustomer_edit('${l.nameString}客户详情','<%=path%>/customer/showcustomer?customer.idString=${l.idString}','${l.nameString}','800','500')">${l.nameString}</a>
						</td>
						<td>${l.genderString}</td>
						<td>${l.identityIdString}</td>
						<td>${l.phoneString}</td>
						<td>${l.emailString}</td>
						<td>
						...
						<c:if test="${l.statusInt==0}">
							...
							<td class="td-manage">
								<a style="text-decoration: none" onClick="customer_stop(this,'${l.idString}')" href="javascript:;"	title="售罄"><i class="Hui-iconfont">&#xe631;</i></a> 
								<a title="删除客户" href="javascript:;" onclick="customer_del(this,'${l.idString}')"	class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6e2;</i></a>
								<a style="text-decoration: none" onClick="house_list(this,'${l.phoneString}')" href="javascript:;"	title="客户奖品"><i class="Hui-iconfont">&#xe687;</i></a> 
								<a title="编辑" href="javascript:;"	onclick="mycustomer_edit('${l.nameString}客户的购房账单','<%=path%>/sale/initaddsale','${l.nameString}','600','500')" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i>确认购房</a> 
							</td>
						</c:if>
						<c:if test="${l.statusInt==1}">
							<td class="td-status"><span class="label label-default radius">已禁用</span></td>
							<td class="td-manage">
								<a style="text-decoration: none"	onClick="customer_start(this,'${l.idString}')" href="javascript:;"	title="开售"><i class="Hui-iconfont">&#xe615;</i></a> 
								<a title="删除客户" href="javascript:;" onclick="customer_del(this,'${l.idString}')" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6e2;</i></a>
								<a style="text-decoration: none" onClick="house_list(this,'${l.phoneString}')" href="javascript:;"	title="客户奖品"><i class="Hui-iconfont">&#xe687;</i></a> 
								<a title="编辑" href="javascript:;"	onclick="mycustomer_edit('${l.nameString}客户的购房的账单','<%=path%>/sale/initaddsale','${l.nameString}','600','500')" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i>确认购房</a> 
							</td>
						</c:if>
					</tr>
				</s:iterator>
			...
			<div class="row-fluid">
				...
				<div class="span6">
					<div class="dataTables_paginate paging_bootstrap pagination">
						<ul>
							<li class="prev">
								<a href="<%=path%>/customer/pagecustomer?currentpage=1&handle=firstpage">← 第一页</a>
							</li>
							<li class="prev">
								<a href="<%=path%>/customer/pagecustomer?currentpage=${pager.currentpage}&handle=uppage">← 上一页</a>
							</li>
<!-- 							<li> -->
<%-- 								<a href="<%=path%>/lp/pagecustomer?currentpage=${n}">${n}</a> --%>
<!-- 							</li> -->
							<li class="next">
								<a href="<%=path%>/customer/pagecustomer?currentpage=${pager.currentpage}&handle=downpage">下一页 → </a>
							</li>
							<li class="next">
								<a href="<%=path%>/customer/pagecustomer?currentpage=${pager.sumpage}&handle=lastpage">尾页 → </a>
							</li>
						</ul>
					</div>
				</div>
...
<script type="text/javascript">
		...
		function checkpage(index){
			window.location.href='${pageContext.request.contextPath}/customer/pagecustomer?currentpage='+index;
		}
		function house_list(obj, id){
			window.location.href='${pageContext.request.contextPath}/getprize/pagecustomerprize?Prize.useridString='+id;
		}
		/*客户-刷新*/
		function refresh(){
			window.location.href='${pageContext.request.contextPath}/customer/pagecustomer';
			layer.msg('刷新成功!', {
				icon : 1,
				time : 2000
			});
		...
		/*楼盘-编辑*/
		function mycustomer_edit(title, url, id, w, h) {
