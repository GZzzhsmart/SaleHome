...
<body>
	...
	<div class="page-container">
		...
		<table
			...
			<thead>
				<tr>
					<th scope="col" colspan="6">客户预约列表</th>
				</tr>
				<tr class="text-c">
					<th width="150">客户头像</th>
					<th width="150">客户名称</th>
					<th width="150">客户员工</th>
					<th width="150">心仪户型</th>
					<th width="130">预约时间</th>
					<c:if test="${l.arriveTime!=null}">
						<th width="130">看房时间</th>
					</c:if>
					<th width="130">预约状态</th>
					<th width="130">状态</th>
				</tr>
			</thead>
			<tbody id="pageBody">
				<s:iterator var="l" value="#request.appointmentlist">
					<tr class="text-c">
						...
						<c:if test="${l.status==0}">
							<td class="td-status"><span class="label label-success radius">已接待</span></td>
						</c:if>
						<c:if test="${l.status==1}">
							<td class="td-status"><span class="label label-default radius">客户放弃预约</span></td>
						</c:if>
					</tr>
				</s:iterator>
			</tbody>
			...
			<div class="row-fluid">
				...
				<div class="span6">
					<div class="dataTables_paginate paging_bootstrap pagination">
						<ul>
							<li class="prev">
								<a href="<%=path%>/appointment/pagejxsappointment?currentpage=1&handle=firstpage">← 第一页</a>
							</li>
							<li class="prev">
								<a href="<%=path%>/appointment/pagejxsappointment?currentpage=${pager.currentpage}&handle=uppage">← 上一页</a>
							</li>
<!-- 							<li> -->
<%-- 								<a href="<%=path%>/lp/pageuser?currentpage=${n}">${n}</a> --%>
<!-- 							</li> -->
							<li class="next">
								<a href="<%=path%>/appointment/pagejxsappointment?currentpage=${pager.currentpage}&handle=downpage">下一页 → </a>
							</li>
							<li class="next">
								<a href="<%=path%>/appointment/pagejxsappointment?currentpage=${pager.sumpage}&handle=lastpage">尾页 → </a>
							</li>
						</ul>
					</div>
				</div>
...
<script type="text/javascript">
		...
		function checkpage(index){
			window.location.href='${pageContext.request.contextPath}/appointment/pagejxsappointment?currentpage='+index;
		}
		/*客户预约-刷新*/
		function refresh(){
			window.location.href='${pageContext.request.contextPath}/appointment/pagejxsappointment';
			layer.msg('刷新成功!', {
				icon : 1,
				time : 2000
			});
